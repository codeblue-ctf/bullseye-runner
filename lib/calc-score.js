...
const setupDockerCompose = (workingDir, dockerCompose, pull=false) => {
  ...
  if (pull) {
    ...
    spawnSyncWithLog('docker', ['network', 'prune', '-f'], { cwd: workingDir })
    const imagesResult = spawnSyncWithLog('docker', ['images', '--digests'])
    const imagesLog = `${imagesResult.stdout.toString()}
-----
${imagesResult.stderr.toString()}`;
    return imagesLog;
  }
}
...
const runExploit = async (i, workingDir, timeout) => {
  ...
  spawnSyncWithLog('docker-compose', ['down', '-t', 3], { cwd: workingDir })
  spawnSyncWithLog('docker-compose', ['kill'], { cwd: workingDir })
...
const runExploits = async (config, flags) => {
  ...
  const workersPromise = workers.map(async (worker, workerIndex) => {
    for (let i = workerIndex; i < config.trials_count; i += workers.length) {
      ...
      const result = setupDockerCompose(workingDir, config.docker_compose, i === 0)
      if (i === 0) {
        imagesLog = result
      }
...
const calcScore = async (config) => {
  ...
  const { submittedFlags, imagesLog } = await runExploits(config, flags)
